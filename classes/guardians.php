...
class Guardians extends DbAccess {
	...
	public function scanUsernameGuardian()
	{
	...
	public function scanGuardianMail()
	{
	...
	private function guardianRegisterFeed(){
	...
	//
	public function bookScheduleCheckAvailablityGuardian(){
		...
		if($this->bookScheduleFeed())
		{
			...
			$availablityCheck = "SELECT * FROM bookings WHERE sc_id ='$sc_id' AND u_type='guardian'";
	...
	public function guardianPushIn()
	{
	 	if($this->guardianFeedPushIn())
	 	{
	...
	private function guardianFeedPushIn()
	{
 	...
 	public function getGuardianSchedules()
	{
	...
	public function getGuardianBookings(){
		$g_id =$_SESSION['g_id'];
		$u_type =$_SESSION['type'];

		$query = "SELECT  b.*,sc.description,sc.description,sc.status AS scstatus,t.first_name AS tfirst,t.last_name AS tlast,t.email AS temail,g.first_name AS gfirst,g.last_name AS glast , g.email AS gemail
					   FROM `bookings` AS b 
					   ...
					   WHERE b.appoint_id = '$g_id' AND b.u_type = '$u_type' ORDER BY b.b_date DESC ";
	...
	public function bookScheduleGuardian(){
		...
		if($this->bookScheduleFeed()){
			...
			$bookingQuery1 = "SELECT * FROM $table WHERE sc_id = '$sc_id' AND u_type = 'student' ORDER BY b_id DESC ";
			  
		


			$result = mysqli_query($this->DBlink,$bookingQuery1) or die("Error Fetching data : ".mysqli_error($this->DBlink));
			$b_ids = "";
			$appoint_ids = "";
	 		if($result)
	 		{
	 			...
	 			if(mysqli_num_rows($result) > 0)
				{
					while($row = mysqli_fetch_assoc($result))
					{
						...
						$timeEndTemp = $row['b_time_end'];
						 ...
						 if (strtotime($timeStartTemp) >= strtotime($time_start) && strtotime($timeEndTemp) <= strtotime($time_end)) 
						 {
						 	...
						 	$b_id_temp =$row['b_id'];
						 	$appoint_id_temp =$row['appoint_id'];

						 	
						 	$attributeSetValues = "status="."'open'";

						   	if($this->updateRecord('students',$attributeSetValues,'st_id',$appoint_id_temp)){
						    	$attributeSetValues = "status="."'reschedule',notification="."'true'";

							   if($this->updateRecord('bookings',$attributeSetValues,'b_id',$b_id_temp)){
					...
					if($this->insertRecord($table,$attribute,$values))
					{
						...
						if($this->updateRecord('schedules',$attributeSetValues,'sc_id',$sc_id))
						{
							...
							if($this->updateRecord('guardians',$attributeSetValues,'g_id',$g_id)){
	...
	private function bookScheduleFeed(){
		...
		if($_POST['data']['g_id'] == "" || empty($_POST['data']['g_id'] )){
