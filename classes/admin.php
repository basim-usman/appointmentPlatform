...
class Admin extends DbAccess {
	...
	public function scanUsernameAdmin()
	{
	...
	private function AdminRegisterFeed(){
	...
	public function addNews(){
		...
		if($this->addNewsFeed()){
			$table = "news";
			$attribute = "title,description,status,added_date,updated_date";

				$values    = "'".$_POST['data']['title']."',
				...
				if($this->insertRecord($table,$attribute,$values)){
					$result=array();
					$result['state']="true";
					$result['error']="Successfully Addeed";

					return $result;
				}			 
		}
	}
	private function addNewsFeed(){

		$state = true;
		if($_POST['data']['title'] == "" || empty($_POST['data']['title'] )){

			$state = false;
		}

		if($_POST['data']['description'] == "" || empty($_POST['data']['description'] )){
	...
	private function AdminFeedPushIn()
	{
	...
	public function getAdminchedules()
	{
