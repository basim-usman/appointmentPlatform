...
class Users extends DbAccess {
	...
	public function letMeIn()
	{
		...
		if($this->userRegisterFeed()){
			

				$table = "users";


				$attribute = "uci,username,email,phone_number,full_name,password,password_text,balance,is_verified,status,added_date,updated_date,f_code,updated_by";
						
				$values    = "'".trim($_POST['data']['generatedUCI'])."',
							'".trim($_POST['data']['username'])."',
							'".trim($_POST['data']['email'])."',
							'".trim($_POST['data']['number'])."',
	...
	private function userRegisterFeed(){
		...
		if($_POST['data']['generatedUCI'] == "" || empty($_POST['data']['generatedUCI'] )){

				$state = false;
		}	

		if($_POST['data']['sCode'] == "" || empty($_POST['data']['sCode'] )){
